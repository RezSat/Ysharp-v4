try:
    import ysharp.errors as errors
    import ysharp.builtin_types as bt
    import ysharp.ysharp_main
...
class Process:
    ...
    def sandbox_statement(self, tree):
        ysharp_main.main(f"{str(self.file_path)}::sandbox", tree=tree[0]["PROGRAM"])
    ...
    def import_statement(self, tree, limport=False):
        ...
        path_search = (
            ...
            or path.isfile(
                path.join(path.dirname(self.file_path), f"{path_items[0]}.yshp")
            )
        ...
        for i, path_bit in enumerate(path_items):
            if path.isdir(path.join(path_search, path_bit)):
                path_search = path.join(
                    path_search, path.join(path_bit, "--init--.yshp")
                )
            elif path.isfile(path.join(path_search, f"{path_bit}.yshp")):
                path_search = path.join(path_search, f"{path_bit}.yshp")
            del path_items[i]

        objects = ysharp_main.get_objects(path_search)
